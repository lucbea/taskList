...
export const FormularioEditar = ({ tareas, setTareas, tareaAEdit, setTareaAEdit, setOpen, setMontarComponente, theme }) => {
    ...
    const editando = () => {
        ...
        if (tareaNuevaParaLS.length > 2 && tareaNuevaParaLS.length < 51) {
    ...
    return (
        <div style={{ padding: '20px', maxWidth: '300px' }}>
            <h4 style={{ textAlign: 'center', color: theme.palette.primary.textColor }}>EDICIÓN DE TAREA</h4>
            <Box
                component="form"
                sx={{
                    display: 'flex',
                ...
                autoComplete="off"
            >
                <p style={{ visibility: error ? "visible" : "hidden", color: theme.palette.primary.colorRed, fontWeight: '900', fontSize: '10px' }}>Obligatorio: la tarea debe tener entre 3 y 50 caracteres</p>
                <textarea
                    id="filled-textarea"
                    defaultValue={tareaAEdit.tarea}
                    onChange={handleChangeTarea}
                    style={{ width: '100%', height: '80px', paddingBlock: '10px', padding: '10px', marginInline: '25px', marginBottom: '13px', border: theme.palette.primary.borderColorInp, color: theme.palette.primary.textColor, backgroundColor: theme.palette.primary.bgContent, border: theme.palette.primary.inputBorderColor }}
                ></textarea>
                <p style={{ visibility: errorFech ? "visible" : "hidden", color: theme.palette.primary.colorRed, fontWeight: '700', fontSize: '10px' }}>Aviso: está eligiendo una fecha del pasado</p>
                <input style={{ marginBlock: '3px', border: theme.palette.primary.inputBorderColor, color: theme.palette.primary.textColor, backgroundColor: theme.palette.primary.bgContent, padding: '5px' }} type="date" defaultValue={tareaAEdit.fechaLim} onChange={handleChangeFecha}>
                </input>
            ...
            <Box sx={{ position: 'relative', justifyContent: 'flex-end', marginTop: '15px', height: '50px' }}>
                ...
                <IconButton onClick={editando} type="submit" edge="end" aria-label="editar" sx={{ ...formTaskStyles.submitBtn, color: theme.palette.primary.colorGreen, position: 'absolute', bottom: '2px', right: '2px' }}>
